...
_SEED_COMPETITOR_KEYWORDS = [
    ...
    # 2026-07-06 2차 대조(6/22~7/6 총 6일치 리포트) 추가 발견분:
    # 롯데렌탈/SK렌터카는 "~중고차" 세부 키워드만 있어 "롯데렌탈, TPG에 팔린다"처럼
    # 중고차 단어가 없는 모기업 뉴스는 놓치고 있었음. 코오롱모빌리티(오토허브셀카 인수 등)와
    # 붕붕마켓(중고차 직거래)도 반복 등장하는데 키워드 자체가 없었음.
    "롯데렌탈", "SK렌터카", "코오롱모빌리티", "붕붕마켓",
...
_SEED_INDUSTRY_KEYWORDS = {
    ...
    "3": [
        ...
        "BYD", "토요타", "렉서스", "닛산",
        # 2026-07-06 2차 대조(6/22~7/6) 추가 발견분: 반복 등장하는 신흥/누락 브랜드
        "폴스타", "지커", "포르쉐", "한국GM", "쉐보레",
...
# -----------------------------------------------------------------------------
DOMESTIC_OEM_BRANDS = ["현대차", "기아", "제네시스", "르노코리아", "KGM", "한국지엠", "한국GM", "쉐보레"]
IMPORT_OEM_BRANDS = ["벤츠", "BMW", "아우디", "볼보", "테슬라", "BYD", "토요타", "렉서스", "닛산", "폴스타", "지커", "포르쉐"]
