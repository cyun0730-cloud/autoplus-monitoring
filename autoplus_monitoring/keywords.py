...
_SEED_INDUSTRY_KEYWORDS = {
    "1": ["중고차", "렌터카", "구독서비스"],
    "2": ["캐피탈+중고차", "카드사"],
    "3": ["벤츠", "BMW", "아우디", "테슬라"],  # 신차출시·판매동향·국내전략만 포함, 단순 프로모션 제외
    "4": ["업계 기획", "업계 트렌드"],
    "5": ["인프라", "법규", "자율주행", "모빌리티"],
}
...
#    - 확인 필요: 인수인계서 원문에는 "현대차 → 제네시스"로 별도 행이 존재하나,
#      COMPETITOR/INDUSTRY 키워드 구조 단순화를 위해 브랜드 뉴스 참고용 리스트로만
#      별도 관리. 실제 검색 키워드는 INDUSTRY_KEYWORDS[3]을 따른다.
# -----------------------------------------------------------------------------
DOMESTIC_OEM_BRANDS = ["현대차그룹(현대&기아)", "제네시스", "기아", "르노코리아", "KGM", "한국지엠"]
IMPORT_OEM_BRANDS = ["벤츠", "BMW", "아우디", "볼보", "테슬라"]
